...
class Walrus:
    ...
    def stakesui(self):
        ...
        try:
            ...
            try:
                suiwindow2.ele(
                    "tag:button@class=appearance-none p-0 bg-transparent border-none cursor-pointer text-steel hover:text-hero-dark ml-auto flex items-center justify-center")
            except Exception:
        ...
        """
        stake_page.ele(
            "tag:button@class=justify-center whitespace-nowrap font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-neutral-950 disabled:pointer-events-none disabled:opacity-50 bg-primaryDark text-primary shadow-sm hover:bg-primaryDark/80 border border-primary/50 hover:border-primary h-8 rounded-md px-3 text-xs flex items-center gap-2").click()
        stake_page.ele(
            "tag:input@class=border-primary/30 focus-visible:ring-secondary flex h-9 w-full rounded-md border bg-transparent px-3 py-1 text-sm shadow-sm transition-colors file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-neutral-950 placeholder:text-neutral-500 focus-visible:outline-none focus-visible:ring-1 disabled:cursor-not-allowed disabled:opacity-50").input(
            1)
        sleep(2)
        ...
        try:
            stake_page.ele(
                "tag:button@class=inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-neutral-950 disabled:pointer-events-none disabled:opacity-50 bg-secondary/90 text-primaryDark shadow hover:bg-secondary h-9 px-4 py-2").click()
            sleep(2)
            """
            捕捉弹窗，确认质押
            """
    ...
    def claimnft(self):
        try:
            ...
            self.claim_page.ele(
                "tag:button@class=rt-reset rt-BaseButton rt-Button rt-r-size-2 rt-variant-surface").click()
...
if __name__ == '__main__':
    ...
    walrus = Walrus(url="http://127.0.0.1:54345", headers={'Content-Type': 'application/json'}, start=start, end=end,
                    password=password)
    walrus.execute()
